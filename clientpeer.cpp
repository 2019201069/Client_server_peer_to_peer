...
// unordered_map <string, set<int>  > userchunkmap;
// unordered_map <string, set<int>  > afterroundrobin;
...
void * senddownloadsignal(void * threadarg){
    ...
    cout<<"yaha tak aya 1"<<endl;
    int ack_len = acks.size();
    for(int i = 0; i < ack_len; i++){
        //userfilechunkmap[downloadinfo->fromclient][downloadinfo->pathfordownload].insert(ack[i]);
        userchunkmap[downloadinfo->fromclient].insert(acks[i]);
        cout<<acks[i]<<", ";    // working fine
    }

    cout<<endl;
    cout<<"yaha tak aya 2"<<endl;
